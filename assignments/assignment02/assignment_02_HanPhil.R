...
## Use `setwd()` if needed
setwd("/User/Phil_Han/Documents/GitHub/dsc520/")
...
#person_df <-read.csv(file = data/tidynomicon/person.csv, header = TRUE, sep = "," )
...
person_df2 <- read.csv("data/tidynomicon/person.csv", header = TRUE, sep = ",")
...
data <- read_excel("data/G04ResultsDetail2004-11-02.xls")
View(data)
excel_sheets("data/G04ResultsDetail2004-11-02.xls")
read_excel("data/G04ResultsDetail2004-11-02.xls")

excel_sheets(readxl_example("data/G04ResultsDetail2004-11-02.xls"))
...
con <-DBI::dbConnect(RSQLite::SQLite(), dbname="data/tidynomicon/example.db")
...
con <-DBI::dbConnect(RSQLite::SQLite(), dbname="data/tidynomicon/example.db")
DBI::dbListTables(con)
DBI::dbGetQuery(con, "SELECT * FROM PERSON")
person_df <- DBI::dbGetQuery(con, "SELECT * FROM PERSON")
person_df
head(person_df)
...
#con <-DBI::dbConnect(RSQLite::SQLite(), dbname="data/tidynomicon/example.db")
lapply(table_names, dbReadTable, conn=db)
